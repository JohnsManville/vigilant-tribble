...
| T4 | Family letter + evidence ledger | Letter at v1.19 (3 fills); ledger v1.2; $20k + $8,837.38 insurance repaid → balance **$30,084.00** (confirmed 2026-08-31). Double-count RESOLVED. | 🟢 | Finish mini harvest; GC hours | **Claude:** finish mini harvest → ledger; reconstruct GC hours. **Jeff:** paper the $15k HELOC draw. |
...
| T14 | Purcell deck matter | Accounting done, attorney email sent 7/22; **Jeff confirms no progress (2026-08-31)** — stalled, not settled. | 🔴 | Anthony McNamer (reply); Jeff (decision to push or shelve) | **Jeff:** decide whether to chase Anthony McNamer for a reply or shelve it; confirm `odd-jobs.org/purcell` is down (OEC 408). No Claude action until Jeff directs. |
...
| T17 | Two-machine sync (lanes + GitHub key) | rsync lane healthy; GitHub lane blocked — MacBook key is a *deploy key* (mumford-week-plan only); other repos on credential-less HTTPS. **Adding an account key can't be done from the Macs — it needs Jeff's GitHub login.** Handoff mirror already routes around it via the mini. | 🔴 | **Jeff (GitHub login — cannot be delegated)** | **Jeff:** at github.com/settings/keys add the key `jeffmumford@macbookpro-whipdesk-setup` as an *account* SSH key (30 sec). **Claude:** then flip the repos' HTTPS remotes → SSH. |
...
| T35 | Handoff-sync mirror + EOD routine (this session) | Mini-driven mirror installed & running; mirrors all three surfaces incl. **Claude Code `~/.claude`**; 5pm PT routine set; first sync done. Plan for all three surfaces: `2026-08-31_three-surface-handoff-plan.md`. | 🟢 | DST (Nov cron shift) | **Claude:** add scattered `~/ClaudeBox/**/HANDOFF*.md` to the sync (Phase 1 of the plan); confirm tomorrow's 5pm run. **Nov:** shift cron `0 0`→`0 1 * * *`. |
