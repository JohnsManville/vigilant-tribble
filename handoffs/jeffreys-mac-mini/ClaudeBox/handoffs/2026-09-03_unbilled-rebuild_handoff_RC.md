...
## Addendum 2 (17:45): review page, invoice draft, PGE deck, hosting
- Review GUI: offline-first file `activity-log-rebuild/review/Odd_Jobs_Unbilled_Review_OFFLINE.html` (3.9 MB; embeds all 30,921 texts from 73 business threads; Messages tab with from/to date-time, thread, work-only filter, Copy as record; per-day Sources under the Daily Log; Show texts jump on every job row). Hosted twin without the raw texts: Claude artifact https://claude.ai/code/artifact/b78569e8-9398-42fe-9872-0264402ba18d. Generator: `scripts/build_review.py` + `scripts/review_template.html` (run after `build_xlsx.py`).
- odd-jobs.org hosting: NOT done. The session's permission classifier blocked the WordPress connector and the Cloudflare deploy repeatedly. Paste-ready block for a Custom HTML page (password-protect it): `review/unbilled-review-wp-block.html` (188 KB, data compressed, no raw texts). Jeff's chosen page password: 5119.
- Fairmount balance invoice DRAFT (not sent): `activity-log-rebuild/INVOICE-DRAFT-Fairmount-balance-2026-09-03.md`. As drafted $8,282 before the boat offset; Addendum 1 (upper parking, $2,375) held.
- PGE upper parking deck: dossier `activity-log-rebuild/PGE-UPPER-PARKING-DECK-2026-09-03.md`. Watcher written but NOT loaded (classifier): `scripts/pge-deck-watch.py` + `scripts/org.mumford.pge-deck-watch.plist`. To arm: `cp scripts/org.mumford.pge-deck-watch.plist ~/Library/LaunchAgents/ && python3 scripts/pge-deck-watch.py --no-text && launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/org.mumford.pge-deck-watch.plist`. It reads chat.db read-only, logs to `activity-log-rebuild/PGE-WATCH-LOG.md`, and texts only Jeff's own number.
